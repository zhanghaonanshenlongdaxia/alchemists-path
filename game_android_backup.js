...
let bossFlash = 0; // screen flash intensity (0-1)
let bossFlashColor = '#ff0000'; // flash color
let bossEntranceTimer = 0; // boss entrance animation timer
...
function setupFloor(biomeIdx, floor){
        ...
        for(var i=1;i<rooms.length;i++){
            ...
            for(var j=0;j<cnt;j++){
                ...
                enemies.push({
                    ...
                    eliteAwakened:false,
                    eliteFlash:0,
                    summonCD:isElite?randInt(180,300):0,
                    chargeCD:isElite?randInt(120,220):0,
                    slamCD:isElite?randInt(150,260):0,
                    charging:false,
                    chargeAngle:0,
                    chargeTimer:0
                });
    ...
    if(isBossFloor){
        ...
        bossFlash=1.0;bossFlashColor='#ff0000';
        screenShake=30;
        for(var bi=0;bi<50;bi++){var ba=Math.random()*Math.PI*2,br=Math.random()*80;particles.push({x:bcx+Math.cos(ba)*br,y:bcy+Math.sin(ba)*br,vx:(Math.random()-0.5)*6,vy:(Math.random()-0.5)*6,life:50,maxLife:50,size:5,color:['#ff0000','#ff4400','#ffaa00','#ff6600'][Math.floor(Math.random()*4)]});}
...
function update(){
    ...
    for(var e of enemies){
        ...
        if(e.isBoss&&bossRef===e){
            ...
            if(bossPhase===0&&e.hp<e.maxHp*0.4){
                ...
                spawnParticles(e.x,e.y,'#ff0000',40);
                screenShake=25;playSound('levelUp');
                bossFlash=0.8;bossFlashColor='#ff0000';
                // Extra ring burst
                for(var ri=0;ri<36;ri++){var ra=Math.PI*2*ri/36;particles.push({x:e.x+Math.cos(ra)*20,y:e.y+Math.sin(ra)*20,vx:Math.cos(ra)*5,vy:Math.sin(ra)*5,life:40,maxLife:40,size:4,color:'#ff4400'});}
            }
            ...
            if(e.charging){
                ...
                if(d<30){
                    ...
                    screenShake=20;bossFlash=0.55;bossFlashColor='#ff6600';playSound('hit');}
                ...
                if(d<40&&e.attackCD<=0){
                    ...
                    spawnParticles(player.x,player.y,'#ee4444',14);
                    screenShake=bossPhase===1?18:12;bossFlash=bossPhase===1?0.5:0.3;bossFlashColor=bossPhase===1?'#ff0000':'#ff4400';playSound('hit');}
                ...
                if(e.slamCD<=0&&d<60){
                    ...
                    if(d<50){
                        ...
                        screenShake=22;bossFlash=0.6;bossFlashColor='#ff8800';playSound('hit');}
                        if(playerStats.hp<=0){
                            if(playerStats.revive){playerStats.revive=false;playerStats.hp=Math.floor(playerStats.maxHp/2);spawnFloat(player.x,player.y-20,T('revived'),'#ffaa00');spawnParticles(player.x,player.y,'#ffaa00',12);activeBuffs=activeBuffs.filter(function(b){return b.effect!=='revive';});}
                            else{state='gameover';return;}
                        }
                    }
                    // Visual slam ring (double ring)
                    for(var si=0;si<30;si++){var sa=Math.PI*2*si/30;particles.push({x:e.x+Math.cos(sa)*50,y:e.y+Math.sin(sa)*50,vx:Math.cos(sa)*3.5,vy:Math.sin(sa)*3.5,life:30,maxLife:30,size:5,color:bossPhase===1?'#ff2200':'#ff8844'});}
                    for(var si=0;si<20;si++){var sa=Math.PI*2*si/20+0.1;particles.push({x:e.x+Math.cos(sa)*20,y:e.y+Math.sin(sa)*20,vx:Math.cos(sa)*2,vy:Math.sin(sa)*2,life:20,maxLife:20,size:3,color:'#ffdd00'});}
            ...
            var maxMinions=bossPhase===1?8:5;
            ...
            if(bossSummonTimer>=summonInterval&&minionCount<maxMinions){
                ...
                // Summon flash effect
                bossFlash=0.45;bossFlashColor='#9922ff';
                screenShake=14;
                // Big burst at boss position
                for(var bi=0;bi<30;bi++){var ba=Math.random()*Math.PI*2,br=Math.random()*60;particles.push({x:e.x+Math.cos(ba)*br,y:e.y+Math.sin(ba)*br,vx:(Math.random()-0.5)*4,vy:(Math.random()-0.5)*4,life:35,maxLife:35,size:4,color:'#cc44ff'});}
                for(var si=0;si<summonCount;si++){
                    var sa=Math.random()*Math.PI*2, sr=60+Math.random()*60;
                    ...
                    if(!isSolid(sx2,sy2)){
                        ...
                        spawnParticles(sx2,sy2,'#cc44ff',12);
                    }
                }
                spawnFloat(e.x,e.y-25,'召唤大军！','#cc44ff');
                playSound('craft');
            }
        ...
        if(canSee||(e.alert&&d<200)){
            e.alert=true;e.alertTimer=180;
            if(e.isElite&&!e.eliteAwakened){
                e.eliteAwakened=true;
                spawnFloat(e.x,e.y-24,'精英出现！','#ff55aa');
                spawnParticles(e.x,e.y,'#ff55aa',22);
                screenShake=Math.max(screenShake,10);
                bossFlash=Math.max(bossFlash,0.22);
                bossFlashColor='#ff55aa';
                for(var eri=0;eri<18;eri++){
                    var era=Math.PI*2*eri/18;
                    particles.push({x:e.x+Math.cos(era)*16,y:e.y+Math.sin(era)*16,vx:Math.cos(era)*2.8,vy:Math.sin(era)*2.8,life:24,maxLife:24,size:3,color:'#ff55aa'});
                }
            }
            ...
            if(e.isElite&&e.charging){
                e.chargeTimer--;
                tryMove(e,Math.cos(e.chargeAngle)*(bossPhase===1?3.8:3.2),Math.sin(e.chargeAngle)*(bossPhase===1?3.8:3.2));
                spawnParticles(e.x,e.y,'#ff66aa',1);
                if(d<34){
                    var eliteChargeDmg=Math.max(1,Math.ceil(e.atk*1.25)-playerStats.def);
                    var actualEliteCharge=applyDamageToPlayer(eliteChargeDmg);
                    if(actualEliteCharge!==false){spawnFloat(player.x,player.y-10,'-'+actualEliteCharge,'#ff5599');
                    spawnParticles(player.x,player.y,'#ff5599',14);
                    screenShake=Math.max(screenShake,14);bossFlash=Math.max(bossFlash,0.32);bossFlashColor='#ff5599';playSound('hit');}
                    e.charging=false;e.chargeCD=randInt(150,240);e.attackCD=35;
                }
                if(e.chargeTimer<=0) e.charging=false;
            } else {
                if(d>28) tryMove(e,Math.cos(e.angle)*espd,Math.sin(e.angle)*espd);
            }
            if(e.isElite&&e.enemyTypeKey==='wolf_pack'&&e.summonCD<=0){
                var eliteMinions=enemies.filter(function(en){return !en.isBoss&&!en.isElite&&dist(en,e)<140;}).length;
                if(eliteMinions<4){
                    e.summonCD=randInt(260,360);
                    bossFlash=Math.max(bossFlash,0.26);bossFlashColor='#bb66ff';
                    screenShake=Math.max(screenShake,9);
                    for(var esi=0;esi<2;esi++){
                        var esa=Math.random()*Math.PI*2, esr=42+Math.random()*36;
                        var ex=e.x+Math.cos(esa)*esr, ey=e.y+Math.sin(esa)*esr;
                        if(!isSolid(ex,ey)){
                            enemies.push({
                                x:ex,y:ey,angle:Math.random()*Math.PI*2,
                                hp:Math.max(2,Math.ceil(e.maxHp*0.22)), maxHp:Math.max(2,Math.ceil(e.maxHp*0.22)),
                                radius:6, alert:true, alertTimer:150,
                                patrolAngle:Math.random()*Math.PI*2, patrolTimer:randInt(60,180),
                                animFrame:0, attackCD:0, atk:Math.max(1,Math.ceil(e.atk*0.45)),
                                isElite:false, isBoss:false, enemyTypeKey:'wolf'
                            });
                            spawnParticles(ex,ey,'#bb66ff',10);
                        }
                    }
                    spawnFloat(e.x,e.y-22,'狼群召来！','#bb66ff');
                    playSound('craft');
                }
            }
            if(e.isElite&&e.enemyTypeKey==='wolf_pack'&&e.chargeCD<=0&&d>90&&d<240&&!e.charging){
                e.charging=true;e.chargeAngle=e.angle;e.chargeTimer=24;e.chargeCD=randInt(180,260);
                spawnFloat(e.x,e.y-20,'突袭！','#ff77aa');
                bossFlash=Math.max(bossFlash,0.16);bossFlashColor='#ff77aa';
                playSound('swing');
            }
            if(e.isElite&&e.enemyTypeKey==='anaconda'&&e.slamCD<=0&&d<55){
                e.slamCD=randInt(180,260);
                var eliteSlamDmg=Math.max(1,Math.ceil(e.atk*0.9)-playerStats.def);
                if(d<52){
                    var actualEliteSlam=applyDamageToPlayer(eliteSlamDmg);
                    if(actualEliteSlam!==false){spawnFloat(player.x,player.y-10,'-'+actualEliteSlam,'#88ff66');
                    spawnParticles(player.x,player.y,'#88ff66',12);
                    screenShake=Math.max(screenShake,13);bossFlash=Math.max(bossFlash,0.28);bossFlashColor='#66cc44';playSound('hit');}
                }
                for(var asi=0;asi<20;asi++){
                    var asa=Math.PI*2*asi/20;
                    particles.push({x:e.x+Math.cos(asa)*26,y:e.y+Math.sin(asa)*26,vx:Math.cos(asa)*2.5,vy:Math.sin(asa)*2.5,life:22,maxLife:22,size:3,color:'#88cc44'});
                }
                spawnFloat(e.x,e.y-20,'绞杀！','#88cc44');
            }
            if(d<30&&e.attackCD<=0){
                var dmg=Math.max(1,e.atk-playerStats.def);
                var actualDmg=applyDamageToPlayer(dmg);
                if(actualDmg!==false){spawnFloat(player.x,player.y-10,'-'+actualDmg,'#ee4444');
                spawnParticles(player.x,player.y,e.isElite?'#ff55aa':'#ee4444',e.isElite?12:4);
                screenShake=e.isElite?10:4;
                if(e.isElite){bossFlash=Math.max(bossFlash,0.24);bossFlashColor='#ff55aa';}
                playSound('hit');
        ...
        if(e.isElite&&e.summonCD>0) e.summonCD--;
        if(e.isElite&&e.chargeCD>0) e.chargeCD--;
        if(e.isElite&&e.slamCD>0) e.slamCD--;
        if(e.attackCD>0) e.attackCD--;
    ...
    if(bossFlash>0) bossFlash*=0.88;
    if(bossEntranceTimer>0) bossEntranceTimer--;
}
...
function renderExpedition(){
    ...
    // Full-screen flash on boss attack
    if(bossFlash>0.02){
        ctx.save();
        ctx.globalAlpha=bossFlash*0.65;
        ctx.fillStyle=bossFlashColor;
        ctx.fillRect(0,0,W2,H2);
        ctx.restore();
    }
    // Enrage phase: pulsing red vignette border
    if(bossPhase===1&&bossRef&&bossRef.hp>0){
        var pulse=0.18+Math.sin(frameCount*0.12)*0.12;
        var vigG=ctx.createRadialGradient(W2/2,H2/2,H2*0.25,W2/2,H2/2,H2*0.8);
        vigG.addColorStop(0,'rgba(0,0,0,0)');
        vigG.addColorStop(1,'rgba(220,0,0,'+pulse.toFixed(2)+')');
        ctx.save();ctx.fillStyle=vigG;ctx.fillRect(0,0,W2,H2);ctx.restore();
        // Red corner flares
        ctx.save();ctx.globalAlpha=pulse*0.7;ctx.fillStyle='#ff0000';
        ctx.fillRect(0,0,6,H2);ctx.fillRect(W2-6,0,6,H2);
        ctx.fillRect(0,0,W2,6);ctx.fillRect(0,H2-6,W2,6);
        ctx.restore();
    }
    // Boss entrance dark overlay
    if(bossEntranceTimer>0){
        var t=bossEntranceTimer/120;
        ctx.save();ctx.globalAlpha=t*0.7;ctx.fillStyle='#000';ctx.fillRect(0,0,W2,H2);
        ctx.globalAlpha=1;ctx.fillStyle='#ff2200';ctx.font='bold 36px monospace';ctx.textAlign='center';
        ctx.shadowColor='#ff0000';ctx.shadowBlur=20;
        ctx.fillText('⚠ BOSS ⚠',W2/2,H2/2);
        ctx.restore();
    }
...
function handleExpeditionPopupClick(cx,cy){
    ...
    if(merchantPopup){
        ...
        for(var i=0;i<shopStock.length;i++){
            var item=shopStock[i];
            var y2=iy+i*(itemH+4)+merchantScrollY;
            var btnW2=70,btnH2=28,btnX=px2+pw2-btnW2-20,btnY=y2+10;
            if(cx>=btnX&&cx<=btnX+btnW2&&cy>=btnY&&cy<=btnY+btnH2&&gold>=item.price){
                gold-=item.price;
                if(item.type==='herb') inventory.herbs[item.key]=(inventory.herbs[item.key]||0)+(item.count||1);
                else if(item.type==='potion'){
                    var r=item.recipe;
...
function handleLabClick(cx,cy){
    ...
    var W=canvas.width,H=canvas.height;
    if(labTab){
        var contentY = labPanelContentY + labScrollY;
        if(labTab==='forge'){
            var startX=W/2-160;
            var fgCy=forgePotionListY;
            for(var i=0;i<inventory.potions.length;i++){
                ...
                if(cx>=btnX&&cx<=btnX+btnW&&cy>=btnY&&cy<=btnY+btnH){
                    var found=forgeSelected.indexOf(i);
                    if(found>=0) forgeSelected.splice(found,1);
                    else if(forgeSelected.length<3) forgeSelected.push(i);
                    playSound('click');
                    return;
                }
            }
            var fbY=forgeButtonY;
            ...
            if(forgeSelected.length===3&&cx>=fbX&&cx<=fbX+fbW&&cy>=fbY&&cy<=fbY+fbH){
                ...
                avgTier=Math.round(avgTier/3);
                var pool=WEAPONS.filter(function(w){return w.tier<=Math.max(1,avgTier+1);});
                if(pool.length===0) pool=[WEAPONS[0]];
                var forged=makeWeapon(pool[randInt(0,pool.length-1)]);
            ...
            var startX2=W/2-160,itemH2=50;
            var shopY=contentY+20;
            for(var si=0;si<labShopStock.length;si++){
                var item=labShopStock[si];
                var y2=shopY+20+si*(itemH2+4);
                var btnX2=startX2+240,btnY2=y2+10,btnW3=70,btnH3=28;
                if(cx>=btnX2&&cx<=btnX2+btnW3&&cy>=btnY2&&cy<=btnY2+btnH3){
                    ...
                    labShopStock.splice(si,1);
                    labMessage=T('boughtItem');labMessageTimer=90;playSound('craft');saveGame();return;
                }
            }
        } else if(labTab==='research'){
            var startX3=W/2-160;
            for(var ri=0;ri<RESEARCH.length;ri++){
                var r2=RESEARCH[ri], lv=researchLevels[r2.id]||0;
                if(lv>=r2.maxLvl) continue;
                var cost=getResearchCost(r2);
                var iy2=contentY+40+ri*44;
                var btnX3=startX3+240,btnY3=iy2+6,btnW4=70,btnH4=28;
                if(cx>=btnX3&&cx<=btnX3+btnW4&&cy>=btnY3&&cy<=btnY3+btnH4){
                    if(gold<cost){labMessage=T('notEnoughGold');labMessageTimer=90;playSound('error');return;}
                    gold-=cost;
                    researchLevels[r2.id]=(researchLevels[r2.id]||0)+1;
                    labMessage=T(r2.key)+' '+T('lvl')+' '+researchLevels[r2.id];
